...
with tap3:
    ...
    time_value = st.multiselect(label="Select Years to analyze",options=data.year.value_counts().index.to_list(),default=data.year.value_counts().index.to_list(),key="weather time")
...
with tap4:
    ...
    map_us_heatmap = md.create_heatmap(data, 39.8283, -98.5795, 4)
    ...
    st.write("     1. Miami is the city with highest (2.43%) no. of road accidents in US (2016-2023).")
    st.write("  2. Houston is the city with 2nd highest (2.19%) no. of road accidents in US (2016-2023).")
    ...
    st.write("  5. About 22.9% of the total accident record in US is only form California")
    st.write("  6. Florida is the 2nd highest (11.48% cases) state for no. road accidents in US.")
    st.write("   7. LOS ANGELES is the county with the highest no. of road accidents (7.1%) of total accidents in US")
